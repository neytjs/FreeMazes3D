...
import {returnMetalTexture, returnLiquidTexture, returnWoodTexture} from "../textures.js";

function potionCauldron(x, z, scene, global_objects, item_id, camera) {
  cloneAndShuffleMasterColor();
  cloneAndShufflePotionColors();

  let pot = MeshBuilder.CreateCylinder("cylinder", {diameterTop: 4, diameterBottom: 2.5, height: 2, tessellation: 8}, scene);
  pot.position.y = 2;
  pot.position.x = x;
  pot.position.z = z;
  ...
  let support1 = MeshBuilder.CreateBox("box", {width: 0.5, height: 5, depth: 0.5}, scene);
  ...
  let support2 = MeshBuilder.CreateBox("box", {width: 0.5, height: 5, depth: 0.5}, scene);
  ...
  let rod = MeshBuilder.CreateCylinder("cylinder", {diameter: 0.1, height: 7, tessellation: 8}, scene);
  rod.position.y = 4.7;
  rod.rotation.z = 1.57;
  rod.position.x = x;
  rod.position.z = z;
  ...
  let cauldronLiquidSound = new Sound("cauldronLiquidSound", "./sound/LavaLoop.wav", scene, null, { loop: true, autoplay: true, volume: 0.50, maxDistance: 50 });
  cauldronLiquidSound.attachToMesh(cauldronBarrier);

  generateMagicPotion((x - 20), (z - 20), potion_colors[0], global_objects, scene);
  generateMagicPotion((x + 20), (z + 20), potion_colors[1], global_objects, scene);
  generateMagicPotion((x + 20), (z - 20), potion_colors[2], global_objects, scene);
  generateMagicPotion((x - 20), (z + 20), potion_colors[3], global_objects, scene);
}
