...
function generateSecretArea(floor_x, floor_y, secret_data, scene, treasure_objects) {
          ...
          for (let k = 0, klength = 7; k < klength; k++) {
            if (TreesData[i].data[j][k] !== "_") {
              if (TreesData[i].data[j][k] === "X") {
                generateTree(k, j, floor_y, floor_x, plus_x, plus_z, scene);
              }
            }
          }
        }
  ...
  if (secret_data.type === "horizontal_up") {
    ...
    secret_ground.material.diffuseColor = new Color3(0, 1, 0);
