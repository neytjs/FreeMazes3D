...
function removeBarrierOb22(buttons, solved, obstacle_objects, forcefield_objects, scene, score, global_language) {
  if (buttons.pushingButton === "pushButton1p22") {
    solved.solvedP22 = true;
    for (let l = 0, llength = forcefield_objects.length; l < llength; l++) {
      for (let n = 0, nlength = obstacle_objects.length; n < nlength; n++) {
        if (obstacle_objects[n].obstacle22_id === forcefield_objects[l].forcefield) {
